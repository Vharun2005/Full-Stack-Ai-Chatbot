...
const Main = () => {
  ...
  const getQuestions = async () => {
    try {
      const getqUestions = await axios.post(
        "http://localhost:3500/api/getQuestions",
        { username: name }
      );
      ...
      if (err.response) {
        ...
        alert("error occured in data" + err);
      }
    }
  };
  ...
  const getUser = async () => {
    try {
      const getUsername = await axios.get("http://localhost:3500/api/getuser");
  ...
  const postQuestions = async (message) => {
    try {
      ...
      setQuestions(newArr);
      await axios.post("http://localhost:3500/api/postquestions", {
        username: name,
        question: message,
      });
    } catch (err) {
